...
class VecBool(values: Array[Boolean]) extends Vec[Boolean] { self =>
  ...
  def foldLeft[@spec(Boolean, Int, Long, Float, Double) B: ST](init: B)(f: (B, Boolean) => B): B =
    VecImpl.foldLeft(this)(init)(f)

  def foldLeftWhile[@spec(Boolean, Int, Long, Float, Double) B: ST](init: B)(f: (B, Boolean) => B)(cond: (B, Boolean) => Boolean): B =
    VecImpl.foldLeftWhile(this)(init)(f)(cond)

  def filterFoldLeft[@spec(Boolean, Int, Long, Float, Double) B: ST](pred: (Boolean) => Boolean)(init: B)(f: (B, Boolean) => B): B =
    VecImpl.filterFoldLeft(this)(pred)(init)(f)

  def filterScanLeft[@spec(Boolean, Int, Long, Float, Double) B: ST](pred: (Boolean) => Boolean)(init: B)(f: (B, Boolean) => B): Vec[B] =
    VecImpl.filterScanLeft(this)(pred)(init)(f)

  def rolling[@spec(Boolean, Int, Long, Float, Double) B: ST](winSz: Int, f: Vec[Boolean] => B): Vec[B] =
    VecImpl.rolling(this)(winSz, f)

  def mapValues[@spec(Boolean, Int, Long, Float, Double) B: ST](f: Boolean => B): Vec[B] = VecImpl.mapValues(this)(f)

  def map[@spec(Boolean, Int, Long, Float, Double) B: ST](f: (Int, Boolean) => B): Vec[B] = VecImpl.map(this)(f)

  def scanLeft[@spec(Boolean, Int, Long, Float, Double) B: ST](init: B)(f: (B, Boolean) => B): Vec[B] = VecImpl.scanLeft(this)(init)(f)

  def zipMap[@spec(Int, Long, Float, Double) B: ST, @spec(Boolean, Int, Long, Float, Double) C: ST](other: Vec[B])(f: (Boolean, B) => C): Vec[C] =
