...
class MatLong(r: Int, c: Int, values: Array[Long]) extends Mat[Long] {
  ...
  def map[@spec(Boolean, Int, Long, Float, Double) B: ST](f: (Int, Int, Long) => B): Mat[B] = MatImpl.map(this)(f)

  def mapValues[@spec(Boolean, Int, Long, Float, Double) B: ST](f: (Long) => B): Mat[B] = MatImpl.mapValues(this)(f)
