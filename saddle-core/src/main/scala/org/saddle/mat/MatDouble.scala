...
class MatDouble(r: Int, c: Int, values: Array[Double]) extends Mat[Double] {
  ...
  def map[@spec(Boolean, Int, Long, Float, Double) B: ST](f: (Int, Int, Double) => B): Mat[B] = MatImpl.map(this)(f)

  def mapValues[@spec(Boolean, Int, Long, Float, Double) B: ST](f: (Double) => B): Mat[B] = MatImpl.mapValues(this)(f)
