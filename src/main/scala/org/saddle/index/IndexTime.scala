...
class IndexTime(val times: Index[Long],
  ...
  def concat[B, C](x: Index[B])(implicit wd: Promoter[DateTime, B, C], mc: CLM[C], oc: ORD[C]) =
  ...
  def join(other: Index[DateTime], how: JoinType) = {
    val tmp = times.join(getTimes(other))
    ReIndexer(tmp.lTake, tmp.rTake, il2it(tmp.index))
  }
  ...
  def map[@spec(Boolean, Int, Long, Double) B: ORD : CLM](f: DateTime => B) =
