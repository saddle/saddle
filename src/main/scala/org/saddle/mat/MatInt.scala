 ...
 */
private[saddle] class MatInt(r: Int, c: Int, values: Array[Int]) extends Mat[Int] {
  def repr = this
  ...
  lazy val cachedT = {
    val matT = new MatInt(numCols, numRows, values.clone())
  ...
  private[saddle] def update(i: Int, v: Int) {
    values(i) = v
  }
